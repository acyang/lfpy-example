...
cell_parameters = {          # various cell parameters,
    'morphology' : 'morphologies/j4a.hoc', # Mainen&Sejnowski, 1996
...
x_cell_pos = np.linspace(-300., 300., n_cells)
...
single_grid_LFPs = []
summed_grid_LFP = np.zeros((n_grid_electrode,int(cell_parameters['tstop'] / cell_parameters['dt'] + 1)))
...
#            synapse.set_spike_times(np.array([20.,40.,60.,80.,100.])
...
if RANK==0:
    ...
    # Morphologies axes:
    plt.axes([0.0, 0.0, 0.5, 1], aspect='equal')
    plt.axis('off')

    for i_cell in range(n_cells):
        cell = LFPy.Cell('morphologies/j4a.hoc',
                         nsegs_method='lambda_f',
                         lambda_f=5)
    ...
    cax = fig.add_axes([0.45, 0.4, 0.01, 0.2], frameon=False)
    LFP = np.max(np.abs(summed_grid_LFP),1).reshape(X.shape)
    ...
    #ax.plot(cell.synapses[0].x, cell.synapses[0].z, 
